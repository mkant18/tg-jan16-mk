...
def create_dict():
    ...
    cur = conn.cursor()
    cur.execute('SELECT id, COALESCE(first_name, "") || " " || COALESCE(last_name, "") FROM users')
    user_dict = dict(cur.fetchall())
    conn.close()
    print(user_dict)
    return user_dict

## Transform the dictionary by removing any values where the username is "None" or "Null" or "null" or "NULL" or "none" or "NONE"
def transform_dict(user_dict):
    for uid, usr in user_dict.copy().items():
        if usr is None or usr.strip() == '':
            del user_dict[uid]
        else:
            continue
    print(user_dict)
...
if __name__ == '__main__':
    ...
    user_dict = transform_dict(user_dict)
    uid_usr(user_dict)
